...
export default function LoanForm({ onClose }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    return products.filter(
      (p) => p.stock_actual > 0 && p.nombre.toLowerCase().includes(q)
    );
  }, [products, search]);
  ...
  async function handleSubmit() {
    ...
    if (!person.trim()) {
      toast.error("Indica la persona");
      return;
    }
    const qty = parseInt(quantity);
    if (!qty || qty <= 0 || qty > selected.stock_actual) {
    ...
    if (error) {
      ...
      toast.success(`Préstamo registrado a ${person.trim()}`);
    }

    await Promise.all([loadProducts(), loadLoans()]);
    setSaving(false);
    onClose();
  }
          ...
          <div className="bg-zinc-800/50 rounded-xl p-3">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-zinc-500">Stock: {selected.stock_actual}</p>
              </div>
              <button
